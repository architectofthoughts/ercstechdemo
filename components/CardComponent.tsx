...
const CardComponent: React.FC<CardProps> = ({ card, playerMana, isAnimating, style, onLongPressStart, onPress }) => {
  ...
  const borderColor = isAffordable 
    ? 'border-botw-gold/40 group-hover:border-botw-gold group-hover:shadow-gold-glow' 
    : 'border-zinc-700 opacity-60 grayscale';
    
  const textColor = isAffordable ? 'text-botw-cream' : 'text-zinc-500';
  const costColor = isAffordable ? 'text-botw-gold' : 'text-zinc-500';
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 bg-botw-slate/90 backdrop-blur-sm border ${borderColor} rounded-sm transition-all duration-300`}>
        {/* Corner Ornaments */}
      ...
      <div className={`relative z-10 w-full h-full p-3 flex flex-col ${textColor}`}>
        ...
        <div className="w-full h-24 my-1 flex items-center justify-center relative group-hover:bg-white/5 transition-colors rounded-sm overflow-hidden">
           <div className={`absolute inset-0 border border-white/5 ${isAffordable ? 'group-hover:border-botw-gold/30' : ''}`}></div>
           {card.type === CardType.ATTACK ? (
               <SwordIcon className={`w-10 h-10 ${isAffordable ? 'text-botw-red drop-shadow-[0_0_8px_rgba(255,62,62,0.6)]' : ''}`} />
           ) : (
               <ShieldIcon className={`w-10 h-10 ${isAffordable ? 'text-botw-blue drop-shadow-[0_0_8px_rgba(77,232,254,0.6)]' : ''}`} />
           )}
        ...
        <div className="mt-2 text-center">
            <h3 className="font-serif font-bold text-sm tracking-wider text-botw-gold uppercase truncate">{card.name}</h3>
        ...
        <div className="mt-auto flex justify-center gap-1 opacity-40">
            <div className="w-1 h-1 rounded-full bg-botw-gold"></div>
            <div className="w-1 h-1 rounded-full bg-botw-gold"></div>
            <div className="w-1 h-1 rounded-full bg-botw-gold"></div>
        </div>
      </div>
